...
# Halufa 1 - with Fruit, 1+ man in sushi1 and 1+ in tortillas
...
#create fitdist for the choosen distribution
...
ifOpen <- function(){
  x <- 0
  time_fruit <- get_global(weddingSimulation, "fruitTime")
  time_sim <- now(weddingSimulation)
  if (time_sim >= time_fruit){
    if (time_sim <= time_fruit + 1) {x <- 1}
  }
  return (x)
}
...
weddingSimulation<- simmer("sim name")%>%
  ...
  add_resource("tortillas",capacity=5,queue_size=Inf)%>%
  ...
  add_resource("Fruits",capacity=0,queue_size =0)%>%
...
fruitPath <- trajectory("fruitPath")%>%
  seize("Fruits", 1)%>%
  # log_("I eat fruits")%>%
  release("Fruits",1)%>%
  set_attribute("disappointed", 0) # not disappointed
...
resetPath <- trajectory("resetPath")%>%
  ...
  set_global(keys = "fruitTime", value = function() get_global(weddingSimulation,key="weddingTime") + get_global(weddingSimulation, key="weddingLength")
             + 30)%>%
  ...
  set_queue_size("sushi1",Inf)%>%set_capacity("sushi1", 2)%>%
  set_queue_size("sushi2",Inf)%>%set_capacity("sushi2", 2)%>%
  set_queue_size("vegs",Inf)%>%set_capacity("vegs", 4)%>%
  set_queue_size("tortillas",Inf)%>%set_capacity("tortillas", 5)%>%
  ...
  set_queue_size("Fruits",0)%>%set_capacity("Fruits", 0)%>%
  ...
  timeout(30)%>%
  set_capacity("Fruits",Inf)%>%
  # log_("Fruit stand is open")%>%
  timeout(1)%>%
  set_capacity("Fruits",0)
...
disaGuests <- sqldf("SELECT COUNT(*) as disa
                    FROM attributeData
                    WHERE key = 'disappointed' and value = 0
                    GROUP BY replication
		    ORDER BY 1 ASC")
...
mean_Measure1_halufa1 <- mean(Measure1)
sd_Measure1_halufa1 <- sd(Measure1)

mean_Measure2_halufa1 <- mean(Measure2)
sd_Measure2_halufa1 <- sd(Measure2)

mean_Measure3_halufa1 <- mean(Measure3)
sd_Measure3_halufa1 <- sd(Measure3)
...
relative_accuracy_measure1_halufa1 <- calc_relative_accuracy(mean_Measure1_halufa1, sd_Measure1_halufa1)
relative_accuracy_measure2_halufa1 <- calc_relative_accuracy(mean_Measure2_halufa1, sd_Measure2_halufa1)
relative_accuracy_measure3_halufa1 <- calc_relative_accuracy(mean_Measure3_halufa1, sd_Measure3_halufa1)

# rough number of needed iterations for each measure
n_needed_measure1_halufa1 <- number_of_replications(relative_accuracy_measure1_halufa1)
n_needed_measure2_halufa1 <- number_of_replications(relative_accuracy_measure2_halufa1)
n_needed_measure3_halufa1 <- number_of_replications(relative_accuracy_measure3_halufa1)
