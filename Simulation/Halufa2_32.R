...
# Halufa 2 - with tortillas2, 1+ man in sushi1 and 1+ in vegs
...
weddingSimulation<- simmer("sim name")%>%
  ...
  add_resource("vegs",capacity=5,queue_size=Inf)%>%
  add_resource("tortillas",capacity=4,queue_size=Inf)%>%
  add_resource("tortillas2",capacity=4,queue_size=Inf)%>%
  add_resource("externalBar",capacity=4,queue_size=Inf)%>%
  add_resource("internalBar1",capacity=4,queue_size=Inf)%>%
...
tortillasPath<-trajectory("tortillasPath")%>%
  ...
  simmer::select(resources=c("tortillas","tortillas2"),  policy=c("shortest-queue-available") )%>%
  seize_selected(amount = 1) %>%
...
mainPath<- trajectory("mainPath")%>%
  ...
  # seize("Fruits",1 , continue = c(FALSE), post.seize = fruitPath)%>% 
...
resetPath <- trajectory("resetPath")%>%
  ...
  set_queue_size("tortillas2",Inf)%>%set_capacity("tortillas", 4)%>%
  set_queue_size("externalBar",Inf)%>%set_capacity("externalBar", 4)%>%
...
mean_Measure1_halufa2 <- mean(Measure1)
sd_Measure1_halufa2 <- sd(Measure1)

mean_Measure2_halufa2 <- mean(Measure2)
sd_Measure2_halufa2 <- sd(Measure2)

mean_Measure3_halufa2 <- mean(Measure3)
sd_Measure3_halufa2 <- sd(Measure3)
...
relative_accuracy_measure1_halufa2 <- calc_relative_accuracy(mean_Measure1_halufa2, sd_Measure1_halufa2)
relative_accuracy_measure2_halufa2 <- calc_relative_accuracy(mean_Measure2_halufa2, sd_Measure2_halufa2)
relative_accuracy_measure3_halufa2 <- calc_relative_accuracy(mean_Measure3_halufa2, sd_Measure3_halufa2)

# rough number of needed iterations for each measure
n_needed_measure1_halufa2 <- number_of_replications(relative_accuracy_measure1_halufa2)
n_needed_measure2_halufa2 <- number_of_replications(relative_accuracy_measure2_halufa2)
n_needed_measure3_halufa2 <- number_of_replications(relative_accuracy_measure3_halufa2)
